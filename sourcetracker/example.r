...
getwd()
setwd("/home/code/R/soucetracer/sourcetracker")
# load sample metadata
metadata <- read.table('data/meta-lake.txt',sep='\t',h=T,row.names=1,check=F,comment='')
#metadata <- read.table('data/meta-lake-sediment.txt',sep='\t',h=T,row.names=1,check=F,comment='')
...
otus <- read.table('data/otus.txt',sep='\t',header=T,row.names=1,check=F,comment='')
...
write.csv(results$proportions,file = "sourcetracker-lake.csv",quote = T)
write.csv(results$proportions,file = "sourcetracker-lake-sediment.csv",quote = T)
...
plot(results, labels[test.ix], type='pie', include.legend=TRUE, env.colors=c('#47697E','#5B7444','#CC6666','#79BEDB','#885588'),cex.main =0.9)
